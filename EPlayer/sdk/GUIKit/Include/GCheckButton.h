...
#ifndef _GCHECKBUTTON_H_
#define _GCHECKBUTTON_H_
...
class GCheckButton : public GWidget
{
public:
    GCheckButton(GVOID);
    GCheckButton(GRECT_T *prRect, GUINT32 u4WFlags = WF_CANFOCUSED|WF_TABSTOP,
                 GUINT32 u4ID = 0xFFFFFFFF);
    virtual ~GCheckButton();
    ...
    GVOID    SetStyle(GCHECKBUTTONSTYLE_T *prStyle);
    GVOID    GetStyle(GCHECKBUTTONSTYLE_T *prStyle);
#endif
    GVOID    SetText(GTCHAR *szText);
    GVOID    SetText(GUINT16 u2StrID);
    GVOID    SetCheck(GBOOL fgCheck);
    GBOOL    IsCheck();
...
typedef struct
{
    GStringData         *m_szText;
#if CFG_GUIKIT_NEW_STYLE
    GCHECKBUTTONSTYLE_T *prStyle;
#endif
} GCHECKBUTTONSPECDATA_T;

#endif //_GCHECKBUTTON_H_